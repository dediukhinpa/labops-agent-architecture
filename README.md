...
bash install.sh          # диалог идентичности → скаффолд → бот → голос → токен → systemd → smoke
...
- **Smoke-тест** в конце `install.sh` / `create-agent`: сессия агента дошла до `Listening for channel`; канал отвечает; `recall`/`swarm` доступны по Bearer; реакции 👀/👌 ставятся.
