...
if [ -n "$TG_PLUGIN_DIR" ]; then
  ask TELEGRAM_BOT_TOKEN "Токен Telegram-бота (создайте у @BotFather → /newbot)" ""
  if [ -n "${TELEGRAM_BOT_TOKEN:-}" ]; then
    ask TELEGRAM_ALLOWED_USER_IDS "Ваш Telegram user_id (узнать у @userinfobot)" ""
...
echo
if [ "$FAIL" = "0" ]; then
  printf "${G}✅ Агент '%s' (%s) готов. Напишите ему в Telegram.${N}\n" "$AGENT_NAME" "$AGENT_ID"
else
  printf "${Y}⚠ Агент '%s' создан, но часть проверок не прошла — см. предупреждения выше.${N}\n" "$AGENT_NAME"
fi
